...
function ProfileEditor({profile,setProfile,notify}){
 ...
 const [showcase,setShowcase]=useState(profile.showcase||['trophies','games','activity']);
 const [links,setLinks]=useState(profile.links||['Trophy Room','Favorite Games']);
 const [favoriteGame,setFavoriteGame]=useState(profile.favoriteGame||'Stellar Divide');
 const [mood,setMood]=useState(profile.mood||'Ready to play');
 const badges=['Founder','Admin','Early Access','Trophy Elite','Beta Tester','Community','Collector','Speedrunner','Completionist','Cloud Pioneer','Party Legend','Plus Veteran'];
 const gradients=['aurora','cosmic','sunset','ocean','neon','royal','midnight','emerald','ember','ice','sakura','monochrome'];
 const frames=['prism','gold','neon','galaxy','minimal','trophy','holo','carbon','crystal','royal'];
 const animations=['pulse','float','shimmer','orbit','breathe','scan','spark','none'];
 const layouts=['classic','cinematic','compact','showcase'];
 const save=()=>{setProfile(p=>({...p,showcase,links,favoriteGame,mood}));notify('Complete profile saved')};
 return <div className="profile-editor profile-studio-v2">
  <div className={'ps5-profile-preview '+profile.gradient+' '+profile.animation+' layout-'+(profile.layout||'cinematic')}>
    <div className="profile-cover-noise"/><div className={'profile-avatar preview frame-'+profile.frame}>{profile.name[0]?.toUpperCase()||'P'}<span className="presence-dot"/></div>
    <div className="ps5-profile-main"><div className="profile-name-line"><h2>{profile.name||'Player'}</h2>{profile.plus&&<span className="profile-plus animated-plus">PLUS</span>}{profile.admin&&<span className="admin-badge">ADMIN</span>}</div><p className="profile-mood">{mood}</p><p>{profile.about}</p><div className="profile-badges"><span>◆ {profile.badge}</span>{profile.admin&&<><span>♛ Founder</span><span>✦ Early Access</span><span>⬡ Trophy Elite</span></>}</div></div>
    <div className="ps5-profile-level"><span>LEVEL</span><strong>128</strong><i><b style={{width:'72%'}}/></i></div>
  </div>
  <div className="studio-tabs">{['Identity','Plus Studio','Showcase','Social','Privacy'].map(x=><button className={tab===x?'active':''} onClick={()=>setTab(x)} key={x}>{x}{x==='Plus Studio'&&<Crown size={13}/>}</button>)}</div>
  {tab==='Identity'&&<div className="editor-grid"><section><h3>Profile identity</h3><label>Online ID<input value={profile.name} maxLength="16" onChange={e=>update('name',e.target.value)}/></label><label>About me<textarea maxLength="160" value={profile.about} onChange={e=>update('about',e.target.value)}/></label><label>Profile mood<input value={mood} maxLength="32" onChange={e=>setMood(e.target.value)}/></label><label>Status<select value={profile.status} onChange={e=>update('status',e.target.value)}><option>Online</option><option>Busy</option><option>Appear Offline</option><option>Looking to Play</option><option>Open to Party</option></select></label></section>
  <section><h3>Primary badge</h3><p className="editor-note">Admin collection: every badge is unlocked.</p><div className="choice-grid badges">{badges.map(x=><button className={profile.badge===x?'active':''} onClick={()=>update('badge',x)} key={x}>◆ <span>{x}</span></button>)}</div></section>
  <section><h3>Favorite game</h3><div className="favorite-game-picker">{games.map(g=><button className={favoriteGame===g.title?'active':''} onClick={()=>setFavoriteGame(g.title)} key={g.id}><i style={{background:g.cover}}/><span>{g.title}</span></button>)}</div></section>
  <section><h3>Profile details</h3><FeatureRows rows={[[profile.plus?'PlayStation Plus Premium':'Standard','Membership'],['Netherlands','Region'],['English / Dutch','Languages'],['128','Player level']]} /></section></div>}
  {tab==='Plus Studio'&&<div className="plus-studio-page"><div className="plus-studio-banner"><Crown size={42}/><div><span>PLAYSTATION PLUS PROFILE STUDIO</span><h2>Make your profile move.</h2><p>PS6OS concept cosmetics. Premium controls below are available only while Plus is active.</p></div><button onClick={()=>update('plus',!profile.plus)}>{profile.plus?'Plus active':'Preview Plus'}</button></div>
   <StudioGroup title="Animated profile gradients" subtitle="Dynamic cover themes">{gradients.map(x=><button className={profile.gradient===x?'active':''} onClick={()=>plusOnly(()=>update('gradient',x))} key={x}><i className={'swatch '+x}/><span>{x}</span>{!profile.plus&&<Lock/>}</button>)}</StudioGroup>
   <StudioGroup title="Premium profile frames" subtitle="Animated and luminous avatar borders">{frames.map(x=><button className={profile.frame===x?'active':''} onClick={()=>plusOnly(()=>update('frame',x))} key={x}><span className={'frame-demo frame-'+x}>L</span><b>{x}</b>{!profile.plus&&<Lock/>}</button>)}</StudioGroup>
   <StudioGroup title="Motion effects" subtitle="Animations shown across profile surfaces">{animations.map(x=><button className={profile.animation===x?'active':''} onClick={()=>plusOnly(()=>update('animation',x))} key={x}><Sparkles/><span>{x}</span>{!profile.plus&&<Lock/>}</button>)}</StudioGroup>
   <StudioGroup title="Profile layouts" subtitle="Change the entire composition">{layouts.map(x=><button className={(profile.layout||'cinematic')===x?'active':''} onClick={()=>plusOnly(()=>update('layout',x))} key={x}><LayoutGrid/><span>{x}</span>{!profile.plus&&<Lock/>}</button>)}</StudioGroup>
   <div className="premium-options"><label><div><strong>Animated PLUS nameplate</strong><span>Gold moving membership mark beside your Online ID.</span></div><button className={profile.plusNameplate!==false?'mini-switch on':'mini-switch'} onClick={()=>plusOnly(()=>update('plusNameplate',profile.plusNameplate===false))}><i/></button></label><label><div><strong>Profile ambience</strong><span>Subtle animated lighting behind your profile card.</span></div><button className={profile.ambience!==false?'mini-switch on':'mini-switch'} onClick={()=>plusOnly(()=>update('ambience',profile.ambience===false))}><i/></button></label><label><div><strong>Rare badge animation</strong><span>Give selected rare badges a subtle shine.</span></div><button className={profile.badgeGlow!==false?'mini-switch on':'mini-switch'} onClick={()=>plusOnly(()=>update('badgeGlow',profile.badgeGlow===false))}><i/></button></label></div>
  </div>}
  {tab==='Showcase'&&<div className="showcase-editor"><div className="showcase-intro"><Trophy/><div><h3>Profile Showcase</h3><p>Choose and reorder what visitors see first.</p></div></div><div className="showcase-modules">{[['trophies',Trophy,'Trophy cabinet','Platinums, rare trophies and completion'],['games',Gamepad2,'Favorite games','Pinned games and completion'],['activity',Activity,'Recent activity','Recently played games'],['captures',Eye,'Featured captures','Screenshots and clips'],['plus',Crown,'Plus journey','Membership streak and Plus stats'],['friends',Users,'Gaming circle','Friends and party activity']].map(([id,Icon,title,text])=><button className={showcase.includes(id)?'active':''} onClick={()=>setShowcase(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id])} key={id}><Icon/><div><strong>{title}</strong><span>{text}</span></div>{showcase.includes(id)?<Check/>:<Plus/>}</button>)}</div><div className="profile-showcase-preview">{showcase.map(id=><article key={id}><span>{id.toUpperCase()}</span><strong>{id==='trophies'?'7 Platinum · 412 total':id==='games'?favoriteGame:id==='activity'?'Stellar Divide · 2h ago':id==='captures'?'6 featured moments':id==='plus'?'214 day Plus streak':'4 friends online'}</strong></article>)}</div></div>}
  {tab==='Social'&&<div className="editor-grid"><section><h3>Quick profile links</h3><p className="editor-note">Plus members can pin extra destinations directly to their profile.</p>{['Trophy Room','Favorite Games','Captures','Game Base','Plus Journey','Recent Activity'].map(x=><label className="editor-toggle" key={x}><span>{x}{!profile.plus&&links.length>=2&&!links.includes(x)?' · Plus':''}</span><button className={links.includes(x)?'mini-switch on':'mini-switch'} onClick={()=>{if(!links.includes(x)&&links.length>=2&&!profile.plus)return notify('Plus unlocks more profile links');setLinks(v=>v.includes(x)?v.filter(y=>y!==x):[...v,x])}}><i/></button></label>)}</section><section><h3>Social card</h3><FeatureRows rows={[[friends?.length||4,'Friends'],['3','Following'],['4','Friends online'],['Late Night Gaming','Current party']]} /></section><section><h3>Profile share</h3><p className="editor-note">Generate a shareable concept profile card.</p><button className="wide-action" onClick={()=>notify('Profile share card generated')}><Cast/> Generate profile card</button><button className="wide-action" onClick={()=>notify('Profile QR concept generated')}><Maximize2/> Generate QR card</button></section><section><h3>Plus identity</h3><div className="membership-card"><Crown/><div><strong>{profile.plus?'Premium identity enabled':'Standard identity'}</strong><span>{profile.plus?'Extra links, layouts, animated cosmetics and showcase controls are unlocked.':'Plus-only social customization is locked.'}</span></div></div></section></div>}
  {tab==='Privacy'&&<div className="editor-grid"><section><h3>Profile visibility</h3>{[['showGames','Show games'],['showTrophies','Show trophy showcase'],['showPlaytime','Show playtime'],['showFriends','Show friends'],['showCaptures','Show featured captures'],['showStatus','Show online activity']].map(([k,l])=><label className="editor-toggle" key={k}><span>{l}</span><button className={profile[k]!==false?'mini-switch on':'mini-switch'} onClick={()=>update(k,profile[k]===false)}><i/></button></label>)}</section><section><h3>Privacy preset</h3><p className="editor-note">Quickly choose who can see profile activity.</p>{['Social & Open','Team Player','Friend Focused','Solo & Focused'].map(x=><button className={'privacy-preset '+(profile.privacyPreset===x?'active':'')} onClick={()=>update('privacyPreset',x)} key={x}><ShieldCheck/><span>{x}</span><ChevronRight/></button>)}</section></div>}
  <div className="editor-save"><button className="primary" onClick={save}><Check/> Save all profile changes</button><span>Profile settings are saved locally in this PS6OS concept.</span></div>
 </div>
}
function StudioGroup({title,subtitle,children}){return <section className="studio-group"><div><h3>{title}</h3><p>{subtitle}</p></div><div className="studio-choice-grid">{children}</div></section>}
